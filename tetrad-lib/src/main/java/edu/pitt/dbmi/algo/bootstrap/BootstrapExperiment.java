...
public class BootstrapExperiment {
	...
	BootstrapSearchAction runner = new BootstrapSearchAction(0, 1, algName, parameters, null, verbose);
	...
	try (OutputStream out = new BufferedOutputStream(
		...
		for (Edge e : complete.getEdges()) {
		    ...
		    String line = "" + n1 + ", " + n2 + ", " + AnilB + ", " + AtoB + ", " + BtoA
	...
	// Write Header
	    String header = "A, B, freq(A-->B), freq(B-->A), freq(A<->B), " + algName + " \n";
	    byte data[] = header.getBytes();
	    out.write(data, 0, data.length);
		...
		for (Edge e : complete.getEdges()) {
		    ...
		    String line = "" + n1 + ", " + n2 + ", " + AtoB + ", " + BtoA + ", " + AbB
			    + ", " + estType + "\n";
		    data = line.getBytes();
		    out.write(data, 0, data.length);
