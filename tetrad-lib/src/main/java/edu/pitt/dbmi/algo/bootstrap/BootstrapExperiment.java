...
public class BootstrapExperiment {
    ...
    private boolean overrideResults = false;

    // Output File handles:
    private PrintWriter outProb = null; // Causality based on direct connectivity
    private PrintWriter outProbDirectedPath = null; // causality based on the existence of directed path
    private PrintWriter outTruth = null; // causality based on the existence of directed path
    private PrintWriter outTruthDirectedPath = null; // causality based on the existence of directed path

    public void setOverrideResults(boolean overrideResults){this.overrideResults = overrideResults;}
    ...
    public BootstrapExperiment(DataSet data, String algorithm, int numBootstrapSamples){
        this.data = data;
    ...
    public void runExperiment(){
        ...
        bootstrapSearchObj.setAlgorithm(algorithm);
        ...
        if (verbose) {
            System.out.println("processing time of total bootstrapping : " + (stop - start) / 1000.0 + " sec");
        }
        ...
        stop = System.currentTimeMillis();
        if (verbose) {
            System.out.println("probDistribution based on Directed Path finished in " + (stop-start) +" ms");
        }
        ...
        if (trueGraph!= null) {
            ...
            stop = System.currentTimeMillis();
            if (verbose) {
                System.out.println("Writing true graph finished in " + (stop - start) + " ms");
            }
            ...
            stop = System.currentTimeMillis();
            if (verbose) {
                System.out.println("Writing true graph based on Directed Path finished in " + (stop - start) + " ms");
            }
    ...
    private int writeTruthBasedOnDirectedPath(Graph trueGraph){
        File truthFileDirectedPath = new File(outputDir, truthFileNameDirectedPath);
        try{
        ...
        for (Edge e : complete.getEdges()) {
            ...
            if(trueGraph.existsDirectedPathFromTo(n1, n2)){
                if (trueGraph.existsDirectedPathFromTo(n2, n1)){
                    // There is a bidirectional causal relation between two nodes according to the true PAG
                    trueType = 3;
                }else {
                    trueType = 1;
                }
                ...
                // There is no causal relation between the two nodes based on the true PAG
                trueType = 0;
            }
            print(n1 + ", " + n2 + ", " + trueType +  "\n", outTruthDirectedPath);
        }
    ...
    private int writeTruthBasedOnDirectedPath(HashMap<String , HashMap> trueGraph){
        // An axuiliary function to write the output file of true causal relationships for the yeast dataset
        // trueGraph is a hashMap that records the adjancyList of true causal relationships
...
//					System.out.println(n1Name+" , "+ n2Name +" bidirected causal relation!");
                    trueType = 3; // This means there is a bidirectional true causal relationship between two nodes
                    // according to the interventional data in yeast!
                    trueGraph.get(n2Name).put(n1Name, (int)trueGraph.get(n2Name).get(n1Name)+1);
                }else {
                    trueType = 1;
                }
                trueGraph.get(n1Name).put(n2Name, (int)trueGraph.get(n1Name).get(n2Name)+1);
    ...
    private int writeProbFile() {
        ...
        print("A, B, freq(A-nil-B), freq(A --> B), freq(B --> A), freq(A o-> B), freq(B o-> A), " +
                "freq(A o-o B), freq(A <-> B), freq(A --- B), "+ algorithm + "_out={0-7}" +" \n", outProb);
    ...
    private int writeProbFileBasedOnDirectedPath() {
        File probFileDirectedPath = new File(outputDir, probFileNameDirectedPath);
        try{
        ...
        // Write Header
        print("A, B, freq(A-->B), freq(B-->A), freq(A<->B), "+ algorithm + " \n", outProbDirectedPath);
        if (verbose) {
            System.out.println("MP: on top of complete Graph!");
        }
        Graph complete = new EdgeListGraph(estGraph.getNodes());
        ...
        for (Edge e : complete.getEdges()) {
            ...
            if(estGraph.existsDirectedPathFromTo(n1, n2)){
                if (estGraph.existsDirectedPathFromTo(n2, n1)){
                    // There is a bidirectional causal relation between two nodes according to the estimated Graph
                    estType = 3;
                }else {
                    estType = 1;
                }
                ...
                // There is no causal relation between two nodes based on estimated Graph (i.e., generated by algorithm e.g., FGES, RFCI, etc.)
                estType = -1;
            }
            print(n1 + ", " + n2 +  ", " + AtoB + ", " + BtoA +", "+ AbB+ ", " +  estType +"\n", outProbDirectedPath);
        }
        return 0;
    }

    private void closeOutputFiles(){
        if (this.outProb != null){this.outProb.close();}
        if (this.outProbDirectedPath != null){this.outProbDirectedPath.close();}
        if (this.outTruth != null){this.outTruth.close();}
        if (this.outTruthDirectedPath != null){this.outTruthDirectedPath.close();}
    }
    ...
    public void setOut(String dirName, String baseFileName){
        ...
        // Ad check availability to check if prob file exist then ignore running!
        ...
        //probFileName: In this file causality is defined based on direct connectivity
        probFileName = "probs_" + baseFileName + ".csv";

        //probFileName_directedPath: In this file causality is defined based on the existence of directed path
        probFileNameDirectedPath = "probs_" + baseFileName + "_directedPath" + ".csv";

        //In this file causality is defined based on the existence of direct connectivity(edge) in the true PAG
        truthFileName = "truth_" + baseFileName + ".csv";

        //In this file causality is defined based on the existence of directed path in the true PAG
        truthFileNameDirectedPath = "truth_" + baseFileName + "_directedPath" + ".csv";
    }
    ...
    public int checkProbFileExists(){
        ...
        if (probFile.exists())
        {
            if (overrideResults) {
                if (verbose) {
                    System.out.println("warning: probFile already exists and it will be override!");
                }
            }else{
                    System.out.println("Erorr: probFile already exists and it cannot be override until you turn on the overrideResults flag!");
                    return -1;
            }
        }
        return 0;
...
//			ges.setDepth(depth);
    ...
    private double getProbability(Edge e) {
        ...
        if (!PAGs.get(0).containsNode(e.getNode1())) throw new IllegalArgumentException();
        if (!PAGs.get(0).containsNode(e.getNode2())) throw new IllegalArgumentException();
    ...
    private double getProbabilityBasedOnDirectedPath(Edge e) {
        ...
        for (Graph g : PAGs) {
            if (e.getEndpoint1() == Endpoint.TAIL && e.getEndpoint2() == Endpoint.ARROW){
                if(g.existsDirectedPathFromTo(e.getNode1(), e.getNode2())) count++;

            }else if (e.getEndpoint1() == Endpoint.ARROW && e.getEndpoint2() == Endpoint.TAIL){
                if(g.existsDirectedPathFromTo(e.getNode2(), e.getNode1())) count++;

            }else if (e.getEndpoint1() == Endpoint.ARROW && e.getEndpoint2() == Endpoint.ARROW){
                if(g.existsDirectedPathFromTo(e.getNode1(), e.getNode2())&&
                        g.existsDirectedPathFromTo(e.getNode2(), e.getNode1())) count++;

            } else if (e.getEndpoint1() == Endpoint.NULL || e.getEndpoint2() == Endpoint.NULL) {
                if (!g.isAdjacentTo(e.getNode1(), e.getNode2())) count++;
