...
public class BootstrapTest {
    ...
    private static void addNodeToGraph(Graph graph, List<Node> nodes,
	    int start, int end) {
	if (start == end) {
	...
	addNodeToGraph(graph, complete.getNodes(), 0, complete.getNodes()
		.size() - 1);
	...
	for (Edge e : complete.getEdges()) {
	    ...
	    if (edge != null) {
		edge.addEdgeTypeProbability(new EdgeTypeProbability(
			EdgeTypeProbability.EdgeType.nil, AnilB));
		edge.addEdgeTypeProbability(new EdgeTypeProbability(
			EdgeTypeProbability.EdgeType.ta, AtoB));
		edge.addEdgeTypeProbability(new EdgeTypeProbability(
			EdgeTypeProbability.EdgeType.at, BtoA));
		edge.addEdgeTypeProbability(new EdgeTypeProbability(
			EdgeTypeProbability.EdgeType.ca, ACtoB));
		edge.addEdgeTypeProbability(new EdgeTypeProbability(
			EdgeTypeProbability.EdgeType.ac, BCtoA));
		edge.addEdgeTypeProbability(new EdgeTypeProbability(
			EdgeTypeProbability.EdgeType.cc, AccB));
		edge.addEdgeTypeProbability(new EdgeTypeProbability(
			EdgeTypeProbability.EdgeType.aa, AbB));
		edge.addEdgeTypeProbability(new EdgeTypeProbability(
			EdgeTypeProbability.EdgeType.tt, AuB));
    ...
    private static void countAdjConfMatrix(int[][] adjAr, List<Edge> edges,
	    Graph truth, Graph estimate, int start, int end) {
	if (start == end) {
    ...
    private static void countEdgeTypeConfMatrix(int[][] edgeAr,
	    List<Edge> edges, Graph truth, Graph estimate, int start, int end) {
	if (start == end) {
	    Edge edge = edges.get(start);
	    Node n1 = truth.getNode(edge.getNode1().toString());
	    Node n2 = truth.getNode(edge.getNode2().toString());
	    ...
	    countEdgeTypeConfMatrix(edgeAr, edges, truth, estimate, mid + 1,
		    end);
	}
    }
