...
/**
 * Runs several algorithm from Tetrad. Documentation is available in the wiki of
 * the Tetrad project on GitHub. This will be replaced by the package
 * tetrad-cli.
 *
...
public final class TetradCmd {
    ...
    private int numBootstrapSamples = 100;
	...
	while (tokenizer.hasToken()) {
	    ...
	    } else if ("-bootstrap".equalsIgnoreCase(token)) {
		this.bootstrap = true;
		try {
		    String argument = tokenizer.nextToken();
		    ...
		    if (argument.startsWith("-")) {
			throw new IllegalArgumentException(
				"'-bootstrap' tag must be followed "
					+ "by an argument indicating the number of sampling tests.");
		    }

		    numBootstrapSamples = Integer.parseInt(argument);

		    if (numBootstrapSamples < 0) {
			throw new IllegalArgumentException(
				"Bootstrap sampling number must be > 0.");
		    }
		} catch (NumberFormatException e) {
		    throw new IllegalArgumentException(
			    "Bootstrap sampling parameter is not a number.");
		}
	    } else {
	...
	try {
		...
		if (initialGraphTxtFilename != null) {
		    initialGraph = GraphUtils.loadGraphTxt(new File(
			    initialGraphTxtFilename));
		}
	    } catch (IOException e) {
		throw new RuntimeException(e);
	    }
	} catch (RuntimeException e) {
	    throw new IllegalStateException("Could not load file at "
		    + file.getAbsolutePath());
	}
    }
	...
	if (bootstrap) {
	    runBootstrap();
	} else if ("pc".equalsIgnoreCase(algorithmName)) {
	    runPc();
	...
	if (seed != null) {
		...
		throw new RuntimeException(
			"Seed must be an integer (actually, long) value.");
	    }
	...
	do {
	    dag = new Dag(GraphUtils.randomGraph(nodes, 0, _numEdges, 30, 15,
		    15, false));
	...
	if (this.covarianceMatrix != null) {
	    throw new IllegalStateException(
		    "Cannot run bootstrap on the covariance Data.");
	}
	...
	if (verbose) {
	    systemPrint("Bootstrap");
	    systemPrint(getKnowledge().toString());
	    systemPrint(getVariables().toString());

	    TetradLogger.getInstance().addOutputStream(System.out);

	    TetradLogger.getInstance().setEventsToLog("info", "independencies",
		    "knowledgeOrientations", "impliedOrientations", "graph");
	...
	BootstrapAlgName algName = BootstrapAlgName.FGES;
	if ("fges".equalsIgnoreCase(algorithmName)) {
	    algName = BootstrapAlgName.FGES;
	...
	} else if ("rfci".equalsIgnoreCase(algorithmName)) {
	    algName = BootstrapAlgName.RFCI;
	    
	    // parameters.set("alpha", alpha);
	    parameters.set("depth", getDepth());
	    parameters.set("completeRuleSetUsed", true);
	    // parameters.set("maxPathLength", maxPathLength);
	} else if ("gfci".equalsIgnoreCase(algorithmName)) {
	    algName = BootstrapAlgName.GFCI;
	    ...
	    parameters.set("depth", getDepth());
	    parameters.set("possibleDsepSearchDone", !nodsep);
	    
	} else {
	    throw new IllegalStateException(
		    "Bootstrap Search does not support the " + algorithmName
			    + " algorithm yet.");
	}

	BootstrapTest bootstrapTest = new BootstrapTest(data, algName,
		numBootstrapSamples);
	bootstrapTest.setParallelMode(true);
	bootstrapTest.setParameters(parameters);
	bootstrapTest.setKnowledge(getKnowledge());
	...
	if (useCovariance) {
		...
		SemBicScore score = new SemBicScore(
			new CovarianceMatrixOnTheFly(data));
	...
	if (verbose) {
	    ...
	    TetradLogger.getInstance().setEventsToLog("info", "independencies",
		    "colliderOrientations", "impliedOrientations", "graph");
	    // TetradLogger.getInstance().setForceLog(true);
	...
	if (this.data != null) {
	    fofc = new FindOneFactorClusters(this.data, this.testType,
		    FindOneFactorClusters.Algorithm.GAP, significance);
	    ...
	    fofc = new FindOneFactorClusters(this.covarianceMatrix,
		    this.testType, FindOneFactorClusters.Algorithm.GAP,
		    significance);
	} else {
	    throw new IllegalStateException("Data did not load correctly.");
	}
	...
	if (useCovariance) {
		...
		if (useConditionalCorrelation) {
		    ...
		    independence = new IndTestConditionalCorrelation(data,
			    significance);
