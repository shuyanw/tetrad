...
//        for (Arrow arrow : sortedArrows) {
//            insert(arrow.getA(), arrow.getB(), new HashSet<>(), 0.000001);
//        }
    ...
    private void calculateArrowsForward(Node a, Node b) {
        ...
        if (removedNodes.contains(b)) return;
        ...
        for (int i = 0; i <= TNeighbors.size(); i++) {
            ...
            while ((choice = gen.next()) != null) {
                ...
                if (Double.isNaN(bump)) {
                    bump = Double.POSITIVE_INFINITY;
//                    for (Node t : T) {
//                        graph.removeEdge(t, b);
//                        clearArrow(t, b);
//
//                    }
//                    removedNodes.add(b);
    ...
    private boolean insert(Node x, Node y, Set<Node> T, double bump) {
        ...
        if (verbose) {
            ...
            String message = graph.getNumEdges() + ". INSERT " + graph.getEdge(x, y) +
                    " " + T + " " + bump + " " + label;
            TetradLogger.getInstance().log("insertedEdges", message);
            out.println(message);
        }
